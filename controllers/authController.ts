...
import { CustomError } from "../routers/authRouter";
export class AuthController {
  ...
  registerNewMember = async (req: Request, res: Response) => {
    if (!req.body.username) {
      throw new CustomError(400, "Incoming Request has no username");
    }
    if (!req.body.password) {
      throw new CustomError(400, "Incoming Request has no password");
    }
    const data = req.body;
  ...
  adminGetItem = async (req: Request, res: Response) => {
    ...
    try {
      const result = await this.authService.adminGetItem(
        Number(product_option_id)
      );
      res.json(result);
    } catch (error) {
  ...
  saveEditProduct = async (req: Request, res: Response) => {
    ...
    try {
      await this.authService.saveEditProduct(
        product_option_id,
        quantity,
        color_id,
        model_id
  ...
  addNewProduct = async (req: Request, res: Response) => {
    ...
    try {
      await this.authService.addNewProduct(
        product_name,
        quantity,
        color_id,
        model_id,
